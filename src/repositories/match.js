  ...
  getAllFinishedAtLastSession() {
    ...
    .then(([max_point, game_point]) => this.db.all(
        `SELECT m.id                                                                                                           AS match_id,
                ...
                (SELECT sum(coalesce(previous, 0)) FROM rating WHERE user_id IN (m.user_1_id, m.user_2_id) AND game_id = g.id) AS rating1,
                (SELECT sum(coalesce(previous, 0)) FROM rating WHERE user_id IN (m.user_3_id, m.user_4_id) AND game_id = g.id) AS rating2,
                (SELECT coalesce(delta, 0) FROM rating WHERE user_id = m.user_1_id AND game_id = g.id)                         AS delta1 -- delta2 вычисляется сменой знака
         FROM match m
                  JOIN user u1 on u1.id = m.user_1_id
                  JOIN user u2 on u2.id = m.user_2_id
                  JOIN user u3 on u3.id = m.user_3_id
