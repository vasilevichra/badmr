...
class Match {
  ...
  getAllFinishedAtLastSession = () => this.getInfo(this.getAllFinishedAtLastSessionRequest);
  getAllByUser = (user_id) => this.getInfo(this.getAllByUserRequest, user_id);

  getInfo(request, user_id = 0) {
    return Promise.all([settingsService.get('max_point'), settingsService.get('game_point')])
    .then(([max_point, game_point]) => request(max_point, game_point, user_id = 0))
    .then(raw => {
  ...
  getAllFinishedAtLastSessionRequest = (max_point, game_point) => this.db.all(
  ...
  getAllByUserRequest = (max_point, game_point, user_id) => this.db.all(
      `SELECT m.id                                                                                                                AS match_id,
              m.created_at                                                                                                        AS match_created_at,
              m.user_1_id                                                                                                         AS user_1_id,
              m.user_2_id                                                                                                         AS user_2_id,
       ...
       WHERE m.tournament_id = (SELECT t.id FROM tournament t WHERE t.current = 1 AND t.available = 1)
         AND m.finished = 1
         AND (u1.id = $user_id OR u2.id = $user_id OR u3.id = $user_id OR u4.id = $user_id)
       ORDER BY m.id DESC`,
      {
        $game_point: game_point,
        $max_point: max_point,
    ...
    return this.db.run(
        `INSERT INTO match (tournament_id,
         ...
         VALUES ((SELECT id FROM tournament WHERE current = 1),
                 ...
                 ?,
                 ?,
                 (SELECT c.id FROM court c JOIN tournament t ON c.tournament_id = t.id WHERE t.current = 1 AND c.available = 1 ORDER BY c.id ASC LIMIT 1))`,
